...
class Dcm_network(object):
	...
	def exec_iwlist(self, iface):
		try:
			iw_result = subprocess.Popen(['sudo', 'iwlist', iface, 'scanning'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	...
	def get_scanning_result(self, iface):
		...
		if flagExec:
			...
			for dict_network in self.parsed_cells:
				network_table.append([dict_network['Name'], dict_network['Address'], dict_network['Channel'], dict_network['Encryption'], dict_network['Quality']])
