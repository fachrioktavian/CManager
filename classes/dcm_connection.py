...
class Dcm_connection(object):
	...
	def exec_dhclient(self, iface):
		...
		cmd_request = [iface]
		try:
			p = subprocess.Popen(['sudo']+program+cmd_release, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			p.wait()
			p = subprocess.Popen(['sudo']+program+cmd_request, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			p.wait()
			return True
		except:
			return False
		...
		try:
			...
			for line in iter(p.stdout.readline, b''):
				if 'sudo' in line and 'wpa_supplicant' in line and iface in line:
					...
					p = subprocess.Popen(['sudo']+program+cmd_kill, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			return True
		except:
			return False
	...
	def connect_profile(self, iface, profile):
		...
		try:
			...
			for line in iter(p.stdout.readline, b''):
				...
				if self.WPA_DISCONNECTED in line or self.WPA_SCAN_FAILED in line:
					...
					if flagKillWpasupp and flagRelDhclient and self.flagReconnect:
						...
						self.debug(2, self.ACT_NEG, 2, 'Error\'s occured while reconnecting interface')
						p.stdout.close()
						break
			...
			flagKillWpasupp = self.kill_wpasupp(iface)
			flagRelDhclient = self.release_dhclient(iface)
			if flagKillWpasupp and flagRelDhclient: self.debug(2, self.ACT_POS, 1, 'Disconnected')
			else: self.debug(2, self.ACT_POS, 1, 'Error\'s occured while disconnecting interface')
