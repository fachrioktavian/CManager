...
class Dcm_app(object):
	...
	LIST_CMD_DASHBOARD_WIZARD_WIFI = ['set', 'del', 'save', 'show', 'use', 'scan', 'connect', 'back', 'help']
	...
	def pre(self):
		if self.dcm_iface!='NULL': return '('+self.loc_now+'|'+self.dcm_iface+') > '
		else: return '('+self.loc_now+') > '
	...
	def action_dashboard_wizard_wifi(self):
		try:
			...
			if self.cmd == 'set' and total_param>=2:
				...
				if var=='name': self.dcm_connection_handler.set_profile_name(value)
				elif var=='ssid':
					value = ' '.join(self.list_param[1::]) 
					self.dcm_connection_handler.set_profile_ssid(value)
				elif var=='type':
					if value in ['Open', 'WPA']: self.dcm_connection_handler.set_profile_type(value)
					else: self.debug(1, self.ACT_NEG, 2, 'type should \'Open\' or \'WPA\'')
				elif var=='passphrase':
					value = ' '.join(self.list_param[1::]) 
					self.dcm_connection_handler.set_profile_passphrase(value)
			elif self.cmd == 'show' and total_param==1:
				if self.list_param[0]=='profile': self.dcm_connection_handler.show_profile()
				elif self.list_param[0]=='options': self.dcm_connection_handler.show_options()
			elif self.cmd == 'save':
				if self.list_param[0]=='profile' and total_param==1:
					flagSave = self.dcm_connection_handler.save_profile()
					if flagSave: self.debug(1, self.ACT_POS, 1, 'Profile\'s saved')
				...
				if self.dcm_iface != 'NULL' and total_param==0:
					...
					if not flagScan: self.debug(1, self.ACT_NEG, 2, 'Error\'s occured while scanning available wifi networks')
			elif self.cmd == 'connect':
				profile = self.list_param[0]
				if self.dcm_iface != 'NULL' and total_param==1:
					self.debug(1, self.ACT_SYSTEM, 0, 'Connecting interface \''+self.dcm_iface+'\' to profile \''+profile+'\'')
