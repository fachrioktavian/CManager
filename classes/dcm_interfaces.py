...
class Dcm_interfaces(object):
	...
	def resolve_Interfaces(self):
		...
		for data in self.ifaces:
			self.dict_ssid[data] = self.dict_mode[data] = self.dict_wname[data] = self.dict_apaddr[data] = '-'
			try:
	...
	def print_ifaces_wireless_table(self):
		...
		self.wireless_table=[['Interface', 'Status', 'IP Address', 'Mask', 'Mode', 'SSID', 'AP Address', 'Wireless Type']]
		for data in self.ifaces_wireless:
			self.wireless_table.append([data, self.dict_status[data], self.dict_ipaddr[data], self.dict_mask[data], self.dict_mode[data], self.dict_ssid[data], self.dict_apaddr[data], self.dict_wname[data]])
	...
	def print_ifaces_ethernet_table(self):
		...
		for data in self.ifaces_ethernet:
			self.ethernet_table.append([data, self.dict_status[data], self.dict_ipaddr[data], self.dict_mask[data]])
	...
	def print_ifaces_localhost_table(self):
		...
		for data in self.ifaces_localhost:
			self.localhost_table.append([data, self.dict_status[data], self.dict_ipaddr[data], self.dict_mask[data]])
